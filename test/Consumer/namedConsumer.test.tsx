...
import { Consumer, MultiProvider, Provider, ResourcesNotProvidedError, useNamedProvider } from '../../src'
...
const ComsumerComponent = () => {
  return (
    <>
      <Consumer<number> name='test-number'>{(number) => <h2 data-testid='number'>{number}</h2>}</Consumer>
      <Consumer<boolean> name='test-boolean'>
        {(boolean) => <h2 data-testid='boolean'>{boolean ? 'true' : 'false'}</h2>}
      </Consumer>
      <Consumer<string> name='test-text'>{(text) => <h2 data-testid='text'>{text}</h2>}</Consumer>
      <Consumer<Counter> name='test-counter'>{(counter) => <h2 data-testid='count'>{counter.count}</h2>}</Consumer>
      <Consumer<CustomData> name='test-custom'>{(customData) => <h2 data-testid='key'>{customData.key}</h2>}</Consumer>
      <Consumer<CustomData> name='test-custom'>
